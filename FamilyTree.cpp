...
namespace family
{
	...
	void Tree::updateNameRelation(const std::string& name_parent, const std::string& name_child, bool addFather)
	{
        ...
        if(addFather)
        {
            parentNode = new NodeTree(name_parent, childNode->getRank() + 1, childNode);
            childNode->setFather(parentNode);
        }
        else // mother
        {
            if(childNode->getRank()-1 < 0)
            {
                parentNode = new NodeTree(name_parent, childNode->getRank() - 1, childNode);
            }
            else
            {
               parentNode = new NodeTree(name_parent, -1 * childNode->getRank() - 1, childNode); 
            }
